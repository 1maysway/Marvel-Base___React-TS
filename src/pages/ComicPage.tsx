...
import "../scss/pages/_comicPage.scss";
import { LinkInTitle, More, MoreItem } from "../types/Types";
...
function ComicPage() {
  ...
  const [fetchAttempts, setFetchAttempts] = useState<{
    result: boolean;
    moreLikeThat: boolean;
    moreSeries: boolean;
  }>({ result: false, moreLikeThat: false, moreSeries: false });

  const fetchResult = async (id: number = comicsId): Promise<Result | null> => {
    const url = `https://gateway.marvel.com:443/v1/public/comics/${id}?${API_AUTH}`;
  ...
  const fetchMoreLikeThat = async (
    text: string,
    id: number = comicsId
  ): Promise<More | null> => {
    const url =
      `https://gateway.marvel.com:443/v1/public/comics?titleStartsWith=${encodeURIComponent(
        text
      )}&orderBy=-onsaleDate&` + API_AUTH;
  ...
  const fetchMoreSeriesByCharacters = async (
    characters: { id: string | number; name: string }[]
  ): Promise<More | null> => {
    const orderBy = "-startYear";
    const urls = characters.map(
      ({ id }) =>
        `https://gateway.marvel.com:443/v1/public/characters/${id}/series?orderBy=${orderBy}&limit=10&` +
        API_AUTH
    );
  ...
  const fetchData = async (): Promise<{
    ...
    const moreLikeThatResponse = await fetchMoreLikeThat(
      resultResponse?.title.split(" (")[0].split(":")[0] || "",
      id
    );
    ...
    const characters = resultResponse.characters.items
      .map((char) => ({
        id: char.resourceURI.split("/").reverse()[0],
        name: char.name,
      }))
      .slice(0, 5);
    ...
    return {
      ...
      moreLikeThat: moreLikeThatResponse,
      moreSeries: moreSeriesResponse,
    };
  };
  ...
  useEffect(() => {
    ...
    CachedData({
      ...
      sessionItemKey: "comic_page_data",
  ...
  const sortMoreByAvalibleImages = (more: More | null) => {
    return {
      ...
      items:
        more?.items.sort((a, b) =>
          a.imageUrl.includes("not_available")
            ? 1
            : b.imageUrl.includes("not_available")
            ? -1
            : 0
        ) || [],
    };
  ...
  result.creators.items.forEach(({ resourceURI, name, role }, index) => {
    let dataItem = dataForResultShowing.find(
      (elm) => elm.role.toLowerCase() === role
    );
    const itemForPush = {
      name,
    ...
    if (!dataItem) {
      dataForResultShowing.push({
        role: role.substring(0, 1).toUpperCase() + role.substring(1),
        index: dataForResultShowing.length,
        items: [itemForPush],
      });
    } else {
  ...
  const resultForShowing = {
    published: convertISOToDate(
      result.dates.find((elm) => elm.type === "onsaleDate")?.date || ""
    ),
    data: dataForResultShowing as [
      { role: string; index: number; items: [{ name: string; link: string }] }
    ],
    description: result.description,
  };

  const slidesForMoreLikeThat =
    sortMoreByAvalibleImages(moreLikeThat).items.map(mapMoreToSlides);

  const slidesForMoreSeries =
    sortMoreByAvalibleImages(moreSeries).items.map(mapMoreToSlides);
  return (
    <div id="comic_page">
      <Grid container className="comic_page_container">
        <Grid item xs={12} marginBottom={"144px"}>
          <section className="comic_page_firstSection">
            <Grid container spacing={8}>
              <Grid item className="comic_page_leftSide" xs={12} md={4}>
                <Grid container className="comic_page_leftSide_container">
                  <Grid
                    item
                    className="comic_page_leftSide_item"
              ...
              <Grid item className="comic_page_rightSide" xs={12} md>
                <Grid container className="comic_page_rightSide_container">
                  <Grid item className="comic_page_rightSide_item">
                    <Grid
                      container
                      className="comic_page_rightSide_item_container"
                    >
                      ...
                      <Grid item className="rightSide_data" xs={12}>
                        ...
                        >
                          ...
                          {resultForShowing.data.map(({ items, role }) => {
                            return (
                              <Grid
                                item
                              ...
                              >
                                ...
                                <Grid
                                  container
                                  className="rightSide_data_item_value_container"
                                >
                                  {items.map(({ name, link }, i) => (
                                    <Grid
                                      item
                                      className="rightSide_data_item_value"
                                      key={link}
                                    >
                                      <Link
                                        to={link}
                      ...
                      {resultForShowing.description && (
                        <Grid item className="rightSide_description" xs={12}>
                          <div
                            className="description_text"
                            dangerouslySetInnerHTML={{
                              __html: resultForShowing.description,
                            }}
                          ></div>
                        </Grid>
                      )}
        ...
        <Grid item xs={12}>
          <section className="more">
            <Grid container className="more_container">
                  ...
                  <section className="moreLikeThat">
                    <Grid container className="moreLikeThat_container">
                      <Grid
                        item
                        xs={12}
                        className="moreLikeThat_text_container"
                      >
                        <h1 className="moreLikeThat_text">
                          {moreLikeThat.title}
                        </h1>
                      </Grid>
                ...
                <Grid item xs={12}>
                  <section className="moreSeries">
                    <Grid container className="moreSeries_container">
                      <Grid item xs={12} className="moreSeries_text_container">
                        <Grid container>
                          <Grid item xs={12} className="moreSeries_text_inner">
                            <h1 className="moreSeries_text">
                              {moreSeries.title}&nbsp;
                            </h1>
                          </Grid>
                          {!isMobile && (
                            <Grid
                              item
                              xs={12}
                              className="moreSeriest_text_links"
                            >
                              <Grid
                                container
                                className="moreSeriest_text_links_container"
                              >
                                {moreSeries.linksInTitle &&
                                  moreSeries.linksInTitle.map(
                                    ({ text, to }, i, array) => (
                                      <Grid
                                        item
                                        className="moreSeries_text_links_inner"
                                        key={text}
                                      >
                                        <Link
                                          to={to}
