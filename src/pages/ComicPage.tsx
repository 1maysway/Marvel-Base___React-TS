...
function ComicPage() {
  ...
  const fetchMoreLikeThat=async(text:string,id:number=comicsId):Promise<More|null>=>{
  ...
  const fetchData = async () => {
    ...
    const cashedComicPageData=sessionStorage.getItem('comic_page_data');
    ...
    const getFromApi=async()=>{
      ...
      sessionStorage.setItem("comic_page_data",JSON.stringify({result:resultResponse,moreLikeThat:moreLikeThatResponse,moreSeries:moreSeriesResponse,cashedTime:Date.now()}))
    }

    if(cashedComicPageData){
      const comicPageData=await JSON.parse(cashedComicPageData);
      const id=parseInt(window.location.pathname.split("/").reverse()[0])||1;

      console.log(comicPageData.result.id,id);
      
      if(comicPageData && Date.now() - parseInt(comicPageData.cashedTime) < 120000 && comicPageData.result.id===id){
        setResult(comicPageData.result)
        setMoreLikeThat(comicPageData.moreLikeThat)
        setMoreSeries(comicPageData.moreSeries);
  ...
  useEffect(() => {
    setComicsId(parseInt(window.location.pathname.split("/").reverse()[0])||1);
    fetchData();
    window.scrollTo(0, 0)
  }, [location]);
            ...
            <Grid container spacing={8}>
              <Grid item className="comic_page_leftSide" xs={12} md={4}>
                <Grid container className="comic_page_leftSide_container">
                  <Grid item className="comic_page_leftSide_item" padding={{xs:"24px",md:"0px"}}>
                      ...
                      <Grid container className="moreSeries_container">
                        <Grid item xs={12} className="moreSeries_text_container">
                          <Grid container>
                            ...
                            { !isMobile &&
                              <Grid item  xs={12} className="moreSeriest_text_links">
